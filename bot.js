  ...
  try {
    ...
    client.on('ready', async () => {
      ...
      try {
        ...
        const ticketMessageChannel = await client.channels.fetch(
          ticketMessageDb.channelID
        );
    ...
    client.on(`message`, async (message) => {
      ...
      if (command.labAuth) {
        try {
          const labUser = await LabUser.findOne({
            discord_user: message.author.id,
          });

          if (!labUser) {
            return message.reply(
              `Please login to the lab to use this command. Use \`help lab-login\` command for help.`
            );
          }
      ...
      if (command.sshKeyRequired) {
        ...
        try {
          ...
          if (sshKey.error) {
            return message.reply(
              `${sshKey.error} Please save a SSH key to your account before creating a VM. Use the \`help update-ssh\` command for help.`
            );
          }
        } catch (error) {
          console.error(error);
          return message.reply(
            `${error} Please save a SSH key to your account before creating a VM. Use the \`help update-ssh\` command for help.`
          );
        }
      }
