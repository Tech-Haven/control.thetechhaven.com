  ...
  async execute(message, args) {
    ...
    try {
      ...
      if (labUser.discord_user) {
        ...
        {
          message.reply(
            `Lab account is already associated with another Discord account. You got caught ;)`
          );
          ...
          return await owner.send(
            `${message.author} tried signing into your lab account. Reset your password!`
          );
        }
      }

      // User hasn't associated theirDiscord account yet, so update their labUser to reference discordUser
      const updatedLabUser = await LabUser.findOneAndUpdate(
      ...
      message.reply(
        `Success! Discord account is now associated with your lab account.`
      );
    } catch (error) {
      console.error(error);
